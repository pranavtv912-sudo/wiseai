...
class InfiniteGridMenu {
  ...
  private init(onInit?: InitCallback): void {
    ...
    // Use exactly 8 vertices (cube corners projected onto a sphere) to represent the 8 unique team members
    const t = 1.0;
    this.instancePositions = [
      ...
      vec3.normalize(normalized, v);
      vec3.scale(normalized, normalized, this.SPHERE_RADIUS);
      return normalized;
    });
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          {/* Active Member Glassmorphic Card */}
          <div
            className={`
              absolute
              bottom-8
            ...
            `}
          >
            <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-2 border-[#3B82F6] shadow-[0_0_15px_rgba(59,130,246,0.3)] shrink-0">
              <img
            ...
            <div className="flex flex-col justify-center text-left">
              <h3 className="text-lg md:text-2xl font-extrabold text-[#3B82F6] tracking-tight leading-tight">
                {activeItem.title}
              </h3>
              <p className="text-white font-medium text-xs md:text-sm mt-1 leading-snug opacity-95 font-sans">
          ...
          {activeItem.link && (
            <div
              ...
              `}
            >
              <p className="select-none relative text-black top-[0px] text-[20px] font-bold">&#x2197;</p>
            </div>
          )}
        </>
      )}
